...
describe 'Fair', ->
  ...
  describe '#subtitle', ->
    ...
    it 'returns some combinations of dates and location if big', ->
      @fair.set(
        ...
        location: { display: '401 Broadway' }
      ).subtitle().should.equal('Feb 1st - Feb 2nd, 401 Broadway')
  ...
  describe '#fetchSections', ->
    ...
    it 'fetches and returns sections', (done) ->
      ...
      Backbone.sync.args[0][1].url.should.match /// /api/v1/fair/.*/sections ///
      Backbone.sync.args[0][2].success [{ section: 'Focus', partner_shows_count: 2 }]
      
  describe '#fetchShows', ->
    
    it 'fetches fair booths with a default size of 3 and sorted by updated at', (done) ->
      ...
      Backbone.sync.args[0][2].data.sort.should.equal '-updated_at'
  ...
  describe '#fetchOptions', ->
    ...
    it 'fetches the fair options', (done) ->
      ...
      Backbone.sync.args[0][1].url.should.match /// /api/v1/search/filtered/fair/.*/options ///
