...
inline void UpdateWatchdogTimer(uint64_t timeout_in_ms) {
  timeout.it_value.tv_sec = timeout_in_ms / 1000;
  timeout.it_value.tv_nsec = (timeout_in_ms % 1000) * 1000 * 1000;
  CHECK_EQ(0, ::timer_settime(timer_id, 0, &timeout, NULL));
}

inline bool IsWatchdogStarted() {
  return profiler_tid != 0;
}

inline void StartWatchdog(v8::Isolate* isolate, uint64_t timeout_in_ms) {
  sigset_t set;
  ::sigemptyset(&set);
  ...
  UpdateWatchdogTimer(timeout_in_ms);
...
const char* StartCpuProfiling(v8::Isolate* isolate, uint64_t timeout_in_ms) {
  if (IsWatchdogStarted()) {
    ...
    if (timeout_in_ms == 0) {
      ...
      StartWatchdog(isolate, timeout_in_ms);
    }
  }
  return NULL;
}
